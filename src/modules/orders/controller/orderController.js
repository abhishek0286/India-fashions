...
exports.placeMultipalOrder = async (req, res) => {
  try {
    const token = req.token;
    console.log("body data",req.body);
    
    let { items, shippingAddress, paymentMethod, paymentStatus, productstatus } = req.body;


if (typeof items === "string") {
  // Remove extra quotes if exist
  items = items.trim();
  ...
  try {
    ...
    return res.send({
      statusCode: 400,
      success: false,
      message: "Invalid items format, must be JSON",
      result: {}
    });
  }
}
    ...
    for (const i of items) {
      const { productId, variantId, categoryId, quantity } = i;

      if (!productId || !variantId || !categoryId || !quantity) {
        return res.send({
          statusCode: 400,
          success: false,
          message: "Each item must have productId, variantId, categoryId, and quantity",
          result: {}
        });
      }
      ...
      const variant = await Variant.findOne({ _id: variantId, status: "Active" });
      if (!variant) {
        return res.send({
          statusCode: 404,
          success: false,
      ...
      if (!variant.stock || variant.stock <= 0) {
        return res.send({
          statusCode: 400,
          success: false,
          message: `Variant of product ${product.name} is out of stock`,
          result: {}
        });
      }
      ...
      if (variant.stock < quantity) {
        return res.send({
          statusCode: 400,
          success: false,
          message: `Only ${variant.stock} items available for product ${product.name}`,
          result: {}
        });
      }

      const itemPrice = variant.originalPrice * quantity;
    ...
    const shippingFee = 10; // TODO: make dynamic
    const GST = 18; // TODO: make dynamic
    const GSTAmount = (totalPrice * GST) / 100;
    const payableAmount = totalPrice + shippingFee + GSTAmount;
